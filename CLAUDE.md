...
32. **Grinder prompt uses targeted pytest for playbook/tool changes** - `factory/agents/grinder_agent.py:select_validation_command()` replaces the hardcoded `python -m pytest tests/ -x -q` for `playbook` and `tool` artifacts. Playbook changes get `python -m pytest tests/test_playbook_loader.py -q`; tool changes with a known module get `python -m pytest tests/test_<name>.py -q`. The full suite is intentionally avoided because it times out at 120s on single-file markdown changes and several fixture suites are unavailable in the E2B sandbox. MeshWiki subtasks still use `src/tests/ -x -q`.
