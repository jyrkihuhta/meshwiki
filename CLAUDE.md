...
32. **Grinder prompt uses targeted pytest for tool changes; playbook artifacts skip pytest** - `factory/agents/grinder_agent.py:select_validation_command()` replaces the hardcoded `python -m pytest tests/ -x -q` for `tool` artifacts. Tool changes with a known module get `python -m pytest tests/test_<name>.py -q`; armory tools without an inferrable module fall back to the broad suite. Playbook `.md` artifacts have no Python test suite (PR #176) and step 5 is the no-op `5. No pytest run ...` directive — `_artifact_intro` documents `python -m pytest tests/test_playbook_loader.py -q` as the optional local fallback when the agent also touched the Python loader. The full suite is intentionally avoided for tool changes because it times out at 120s and several fixture suites are unavailable in the E2B sandbox. MeshWiki subtasks still use `src/tests/ -x -q`.
