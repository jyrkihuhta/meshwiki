...
def test_parse_suggestions_plain_json():
    out = """[
      {"target":"dashql","vuln_class":"host-header","title":"X","rationale":"y","test_surface":"z"}
    ]"""
...
def test_parse_suggestions_with_markdown_fence():
    out = "```json\n[{\"target\":\"notekeeper\",\"vuln_class\":\"x\",\"title\":\"y\"}]\n```"
    parsed = _parse_suggestions(out, cap=5)
    assert len(parsed) == 1
...
def test_parse_suggestions_drops_invalid_target():
    out = """[
      {"target":"ssrfbox","vuln_class":"a","title":"good"},
      {"target":"out-of-scope","vuln_class":"b","title":"bad"},
      {"target":"notekeeper","vuln_class":"c","title":"also good"}
    ]"""
    parsed = _parse_suggestions(out, cap=5)
...
def test_parse_suggestions_drops_missing_keys():
    out = """[
      {"target":"ssrfbox","vuln_class":"a","title":"good"},
      {"target":"ssrfbox","title":"missing vuln_class"},
...
def test_parse_suggestions_respects_cap():
    out = """[
      {"target":"ssrfbox","vuln_class":"a","title":"1"},
      {"target":"ssrfbox","vuln_class":"b","title":"2"},
      {"target":"ssrfbox","vuln_class":"c","title":"3"},
      {"target":"ssrfbox","vuln_class":"d","title":"4"}
    ]"""
...
async def test_run_deduplicates_against_existing_and_open():
    ...
    bot._fetch_armory_classes = AsyncMock(return_value=[
        ("dashql", "cache-deception"),
        ("ssrfbox", "xxe-blind-oob"),
    ])
    bot._fetch_open_tasks = AsyncMock(return_value=[
        {"target": "notekeeper", "vuln_class": "prototype-pollution",
    ...
    bot._call_llm = AsyncMock(return_value=(
        '[{"target":"dashql","vuln_class":"cache-deception","title":"DUPE armory"},'
        '{"target":"notekeeper","vuln_class":"prototype-pollution","title":"DUPE task"},'
        '{"target":"ssrfbox","vuln_class":"smuggling","title":"NEW one",'
...
async def test_run_returns_zero_when_all_suggestions_are_duplicates():
    ...
    bot._fetch_armory_classes = AsyncMock(return_value=[("dashql", "x")])
    bot._fetch_open_tasks = AsyncMock(return_value=[])
    bot._call_llm = AsyncMock(return_value=(
        '[{"target":"dashql","vuln_class":"x","title":"dup"}]', 0,
    ))
    result = await bot.run()
